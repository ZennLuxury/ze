...
class WAConnection extends _5_User_1.WAConnection {
    ...
    async prepareMessageMedia(media, mediaType, options = {}) {
        ...
        const { mediaKey, encBodyPath, bodyPath, fileEncSha256, fileSha256, fileLength, didSaveToTmpPath } = await (0, Utils_1.encryptedStream)(media, mediaType, requiresOriginalForSomeProcessing);
        ...
        if (requiresThumbnailComputation) {
            await (0, Utils_1.generateThumbnail)(bodyPath, mediaType, options);
        }
        if (requiresDurationComputation) {
            try {
                options.duration = await (0, Utils_1.getAudioDuration)(bodyPath);
            }
            catch (error) {
        ...
        for (let host of json.hosts) {
            ...
            try {
                const { body: responseText } = await this.fetchRequest(url, 'POST', (0, fs_1.createReadStream)(encBodyPath), options.uploadAgent, { 'Content-Type': 'application/octet-stream' });
    ...
    prepareMessageFromContent(id, message, options) {
        if (!message.timestamp)
            message.timestamp = new Date(); // set timestamp to now
        if (typeof options.sendEphemeral === 'undefined')
            message.sendEphemeral = 'chat';
        if (options.viewOnce)
            message = { viewOnceMessage: { message } };
        // prevent an annoying bug (WA doesn't accept sending messages with '@c.us')
        id = (0, Utils_1.whatsappID)(id);
        const key = Object.keys(message)[0];
        const timestamp = (0, Utils_1.unixTimestampSeconds)(options.timestamp);
        ...
        const messageJSON = {
            key: {
                remoteJid: id,
                fromMe: true,
                id: (options === null || options === void 0 ? void 0 : options.messageId) || (0, Utils_1.generateMessageID)(),
            },
            message: message,
    ...
    async relayWAMessage(message, { waitForAck } = { waitForAck: true }) {
        ...
        if (waitForAck) {
            ...
            const emitUpdate = (status) => {
                ...
                this.emit('chat-update', { jid: message.key.remoteJid, messages: (0, Utils_1.newMessagesDB)([message]) });
            };
            promise
    ...
    async downloadAndSaveMediaMessage(message, filename, attachExtension = true) {
        const extension = (0, Utils_1.extensionForMediaMessage)(message.message);
    ...
    async refreshMediaConn(forceGet = true) {
...
__decorate([
    (0, Mutex_1.Mutex)(message => { var _a; return (_a = message === null || message === void 0 ? void 0 : message.key) === null || _a === void 0 ? void 0 : _a.id; })
