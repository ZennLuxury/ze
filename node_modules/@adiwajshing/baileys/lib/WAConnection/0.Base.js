...
class WAConnection extends events_1.EventEmitter {
    ...
    async waitForMessage(tag, requiresPhoneConnection, timeoutMs) {
        ...
        if (requiresPhoneConnection) {
            ...
            cancelPhoneChecker = this.exitQueryIfResponseNotExpected(tag, err => onErrr(err));
        }
        try {
            const result = await Utils.promiseTimeout(timeoutMs, (resolve, reject) => {
                onRecvo = resolve;
                onErrr = ({ reason, status }) => reject(new Constants_1.BaileysError(reason, { status }));
                this.on(`TAG:${tag}`, onRecvo);
                this.on('ws-close', onErrr); // if the socket closes, you'll never receive the message
            });
            return result;
        }
        finally {
            requiresPhoneConnection && this.clearPhoneCheckInterval();
            this.off(`TAG:${tag}`, onRecvo);
            this.off(`ws-close`, onErrr );
